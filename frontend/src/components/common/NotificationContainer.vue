...
function getNotificationClasses(notification: Notification) {
  const base = 'flex items-start gap-3 p-4 rounded-lg shadow-lg border'
...
<template>
  <Teleport to="body">
    <div class="fixed top-4 right-4 z-[70] flex flex-col gap-2 max-w-sm w-full">
      <TransitionGroup name="notification">
        ...
        >
          <!-- Icon -->
          <svg
            :class="['h-5 w-5 flex-shrink-0', getIconColor(notification.type)]"
            fill="currentColor"
            viewBox="0 0 20 20"
          >

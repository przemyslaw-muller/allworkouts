...
class TestListWorkoutSessions:
    ...
    def test_list_workout_sessions_pagination(
        ...
        response = client.get(
            '/api/v1/workout-sessions?page=1&limit=5', headers=auth_headers
        )
...
class TestGetWorkoutSession:
    ...
    def test_get_workout_session_not_found(self, client: TestClient, auth_headers: dict):
        ...
        fake_id = uuid.uuid4()
        response = client.get(
            f'/api/v1/workout-sessions/{fake_id}', headers=auth_headers
        )
...
class TestStartWorkoutSession:
    ...
    def test_start_workout_session_success(
        ...
        response = client.post(
            ...
            json={'workout_plan_id': str(test_workout_plan_with_exercises.id)},
            headers=auth_headers,
        )
    ...
    def test_start_workout_session_nonexistent_plan(
        self, client: TestClient, auth_headers: dict
    ):
        '''Test starting session with non-existent plan.'''
        fake_id = uuid.uuid4()
        response = client.post(
            '/api/v1/workout-sessions/start',
            json={'workout_plan_id': str(fake_id)},
            headers=auth_headers,
        )
    ...
    def test_start_workout_session_other_user_plan(
        ...
        '''Test starting session with another user's plan.'''
        response = client.post(
            '/api/v1/workout-sessions/start',
            json={'workout_plan_id': str(test_workout_plan.id)},
            headers=auth_headers_user2,
        )
...
class TestDeleteWorkoutSession:
    ...
    def test_delete_workout_session_success(
        ...
        response = client.delete(
            f'/api/v1/workout-sessions/{session.id}', headers=auth_headers
        )

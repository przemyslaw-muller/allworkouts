...
@pytest.fixture
def test_equipment(db: Session) -> Equipment:
...
@pytest.fixture
def test_equipment_2(db: Session) -> Equipment:
...
@pytest.fixture
def test_exercise_2(db: Session) -> Exercise:
...
@pytest.fixture
def test_exercise_with_equipment(db: Session, test_exercise: Exercise, test_equipment: Equipment) -> Exercise:
...
@pytest.fixture
def test_workout_session(db: Session, test_user: User, test_workout_plan: WorkoutPlan) -> WorkoutSession:
...
def cleanup_test_data(db: Session, user: User) -> None:
    '''Clean up test data for a specific user.'''
    # Delete personal records
    db.query(PersonalRecord).filter(PersonalRecord.user_id == user.id).delete()
    
    # Delete exercise sessions (through workout sessions)
    session_ids = [
        s.id for s in db.query(WorkoutSession).filter(WorkoutSession.user_id == user.id).all()
    ]
    if session_ids:
        db.query(ExerciseSession).filter(ExerciseSession.workout_session_id.in_(session_ids)).delete()
    
    # Delete workout sessions
    db.query(WorkoutSession).filter(WorkoutSession.user_id == user.id).delete()
    
    # Delete workout exercises (through workout plans)
    plan_ids = [
        p.id for p in db.query(WorkoutPlan).filter(WorkoutPlan.user_id == user.id).all()
    ]
    if plan_ids:
        db.query(WorkoutExercise).filter(WorkoutExercise.workout_plan_id.in_(plan_ids)).delete()
    
    # Delete workout plans
    db.query(WorkoutPlan).filter(WorkoutPlan.user_id == user.id).delete()
    
    # Delete user equipment
    db.query(UserEquipment).filter(UserEquipment.user_id == user.id).delete()
